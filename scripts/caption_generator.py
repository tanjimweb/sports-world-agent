...
a Telegram cross-promotion line for EACH post, using Gemini.

Only Facebook needs captions (Telegram posts just the image), so this
output is consumed later by facebook_poster.py only.

Runs as part of the daily "generate content" workflow, AFTER
content_agent.py and image_generator.py.
...
FIRST_CALL_BUFFER_SECONDS = 20
BETWEEN_CALL_DELAY_SECONDS = 15
